...
async function setupAdmins() {
  // Bilgileri .env dosyasından al (örneğin: ADMIN_EMAIL, ADMIN_PASSWORD)
  ...
  if (!adminEmail || !adminPassword) {
    ...
    console.log("ADMIN_EMAIL=test@test.com ADMIN_PASSWORD=Sifre123! node scripts/create-admins.js");
    console.log("----------------------------------------------------------------");
    return;
  }
  ...
  for (const admin of admins) {
    ...
    // The DB trigger 'on_user_role_change' will automatically sync this to auth.users.raw_app_meta_data
    const { error: roleError } = await supabase
      .from('user_roles')
      .upsert({
        user_id: user.id,
        role: 'admin', // Use 'superadmin' for specific users if needed
      }, { onConflict: 'user_id' });
